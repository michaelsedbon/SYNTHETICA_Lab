...
Usage:
    ...
    python3 pdf_to_text.py --batch <pdf_dir> [output_dir]
...
PROJECT_DIR = os.path.dirname(os.path.dirname(SCRIPT_DIR))  # scripts/ is inside .agents/scripts/
...
def batch_convert(pdf_dir, output_dir=None):
    """Convert all PDFs in a directory (and subdirectories) to text."""
    if output_dir is None:
        output_dir = DEFAULT_OUTPUT_DIR
    ...
    for root, dirs, files in os.walk(pdf_dir):
        ...
        # Determine relative path for output subdirectory
        rel_path = os.path.relpath(root, pdf_dir)
        out_subdir = os.path.join(output_dir, rel_path) if rel_path != '.' else output_dir
        os.makedirs(out_subdir, exist_ok=True)

        existing = set(os.listdir(out_subdir))
        ...
        for pdf_file in sorted(pdf_files):
            ...
            try:
                pdf_to_text(pdf_path, out_subdir)
                converted += 1
