...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    ...
    let appURL = URL(string: "http://172.16.1.80:3000")!
    ...
    func setupWindow() {
        ...
        window.center()
        window.title = "Fab Planner — Production"
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            guard let data = data, error == nil else {
                ...
                DispatchQueue.main.async {
                    self.webView.evaluateJavaScript("window.__nativeDownloadResult?.('error')", completionHandler: nil)
                }
                return
            }
            ...
            do {
                ...
                // Notify JS that download completed
                DispatchQueue.main.async {
                    let escapedPath = savePath.path.replacingOccurrences(of: "'", with: "\\'")
                    self.webView.evaluateJavaScript("window.__nativeDownloadResult?.('success', '\(escapedPath)')", completionHandler: nil)
                    // Reveal in Finder
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationResponse: WKNavigationResponse, decisionHandler: @escaping (WKNavigationResponsePolicy) -> Void) {
        // Also intercept Content-Disposition: attachment responses as a fallback
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        ...
        let html = "<html><body style='background:#1a1a1a;color:white;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;'><div><h1 style='color:#ff3b30;'>Fab Planner</h1><p>Connecting to production server... (Retrying)</p></div></body></html>"
