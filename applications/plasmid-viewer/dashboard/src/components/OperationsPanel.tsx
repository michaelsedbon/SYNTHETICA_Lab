...
import { ChevronRight, Dna, Search, Pencil, Trash2, Crosshair } from "lucide-react";

// Feature types common in synthetic biology
const FEATURE_TYPES = [
...
interface Props {
    ...
    onDataChange: () => void; // refresh data after mutations
}
            ...
            <div className="panel-content" style={{ overflowY: "auto" }}>
                            ...
                            <div className="form-group">
                                ...
                                <div className="checkbox-group">
                                    <label className="checkbox-item">
                                        <input type="checkbox" checked={orfStartATG} onChange={(e) => setOrfStartATG(e.target.checked)} />
                                        ATG
                                    </label>
                                    <label className="checkbox-item">
                                        <input type="checkbox" checked={orfStartGTG} onChange={(e) => setOrfStartGTG(e.target.checked)} />
                                        GTG
                                    </label>
                                    <label className="checkbox-item">
                                        <input type="checkbox" checked={orfStartTTG} onChange={(e) => setOrfStartTTG(e.target.checked)} />
                                        TTG
                                    </label>
                                </div>
                            </div>
                            ...
                            {orfResults.length > 0 && (
                                <>
                                    <div style={{ fontSize: 11, color: "var(--text-tertiary)", marginBottom: 6 }}>
                                        Found {orfResults.length} ORF{orfResults.length !== 1 ? "s" : ""}
                                    </div>
                                    <div className="orf-list">
                                        {orfResults.map((orf, i) => (
                                            <div key={i} className="orf-item">
                                                ...
                                                <div className="orf-info">
                                                    Frame {orf.frame + 1}{orf.strand === -1 ? "r" : "f"} · {orf.start}–{orf.end}
                                                </div>
                ...
                <div className="section">
                    <div className="section-header" onClick={() => setEditOpen(!editOpen)}>
                        ...
                        <h3>Annotation {selectedFeature ? `· ${selectedFeature.label || "Untitled"}` : ""}</h3>
                    </div>
                    {editOpen && (
                        <div className="section-body">
                            {selectedFeature ? (
                                <>
                                    ...
                                    <div className="form-group">
                                        ...
                                        <select
                                            className="form-select"
                                            value={editStrand}
                                            onChange={(e) => setEditStrand(Number(e.target.value))}
                                        >
                                    ...
                                    <div className="btn-group" style={{ marginTop: 16 }}>
                                        <button className="btn btn-primary" onClick={handleSaveFeature}>
                                            <Pencil size={12} /> Save
                                        </button>
                                        <button className="btn btn-danger" onClick={handleDeleteFeature}>
                                ...
                                <div style={{ color: "var(--text-tertiary)", fontSize: 12, padding: "8px 0" }}>
                                    Click a feature on the map or in the list to edit it
                                </div>
                            )}
                        </div>
                    )}
